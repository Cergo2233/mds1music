...
class Music(commands.Cog):
    ...
    async def send_queue(ctx, songs):
        ...
        for i in songs:
            ...
            message += title + '\n'
        if message != '':
            output = str(f"```Зараз у черзі:\n{message}```")
            await ctx.send(output)

    @staticmethod
    async def user_is_connected(ctx):
        if ctx.author.voice is None:
            await ctx.send("```Підключись до голосового каналу, бовдур! >.<```")
            return False
        else:
            return True
    ...
    async def resume(self, ctx):
        if await self.user_is_connected(ctx) and self.voice_channel.is_connected()\
            ...
            await ctx.send("```Відновила програвання!```")
            self.voice_channel.resume()

    @commands.command(pass_context=True)
    async def skipall(self, ctx):
        if await self.user_is_connected(ctx) and self.voice_channel.is_connected() and self.playing:
            await ctx.send("```Ах-ти скіпальщик'~```")
            self.queue = []
            self.loop = False
            self.voice_channel.stop()
    ...
    async def remove(self, ctx, number):
        ...
        message = str(f"```Виладила з черги:\n{title}```")
